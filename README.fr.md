...
[English](README.md) | [العربية](README.ar.md) | [Türkçe](README.tr.md) | **Français** | [中文](README.zh-CN.md) | [Deutsch](README.de.md) | [हिन्दी](README.hi.md)
...
> ⚠️ **D'anciens jetons de bot et l'ancienne clé API partagée sont encore lisibles dans ce dépôt** — dans l'historique git, car ce fichier était autrefois suivi, et dans un fichier source suivi : l'enregistrement de `TelegramLoggerService` dans `TallaEgg.TelegramBot.Infrastructure/Program.cs`, qui n'a pas de clé de configuration propre. **Chacun d'eux a été renouvelé dans le cadre de [#33](https://github.com/MohKardan/TallaEgg/issues/33) et est désormais invalide — ne les signalez pas comme une fuite active.** Les nouveaux lecteurs les redécouvrent à chaque passage et déclenchent un incident ; c'est une fausse alerte à chaque fois. L'historique n'est délibérément **pas** réécrit — cette décision et sa justification figurent dans [#105](https://github.com/MohKardan/TallaEgg/issues/105). Le fichier de configuration lui-même est désormais ignoré par git ; ne le rajoutez pas, ni aucun nouveau secret, au contrôle de version.
