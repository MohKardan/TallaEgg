...
نسخه‌های اولیهٔ این سند یک کلید `Matching:MarketMakerUserId` را لازم می‌دانستند. `GetMode`
تنها همان دو کلید بالا را می‌خواند و آن کلید هیچ‌کدامشان نیست (#200).
